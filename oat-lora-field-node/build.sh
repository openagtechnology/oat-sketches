#!/usr/bin/env bash
# Build the OAT LoRa Field Node for both Heltec boards and assemble the web-installer payload
...
VERSION="$(grep -oE 'FW_SEMVER +"[^"]+"' oat_lora_field_node.ino | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"

echo "==> OAT LoRa Field Node v${VERSION}"
...
cp oat_lora_field_node.ino "${ASSETS}/"
python3 - "${ASSETS}" <<'PY'
import zipfile, sys, os
dst = sys.argv[1]
src = ["oat_lora_field_node.ino", "platformio.ini", "merge_bin.py", "make_manifest.py", "build.sh", "docker-build.sh", "README.md"]
...
with zipfile.ZipFile(os.path.join(dst, "oat-lora-field-node-firmware.zip"), "w", zipfile.ZIP_DEFLATED) as z:
    for f in src:
        if os.path.exists(f): z.write(f, arcname="oat-lora-field-node/" + f)
    for f in libs:
