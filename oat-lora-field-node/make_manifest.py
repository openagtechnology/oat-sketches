...
def main():
    ...
    builds = []
    per_board = []
    ENV_FAMILY = {"heltec-v2": "ESP32", "heltec-v3": "ESP32-S3", "heltec-v4": "ESP32-S3"}
    ...
    # V3 and V4 share a family: the combined manifest keeps the V3 and V2 builds; the V4 is reached by its own button.
    _write(os.path.join(dest, "manifest.json"), dict(common, builds=[b for m, b in per_board if m != "heltec-v4"]))
