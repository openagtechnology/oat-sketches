...
def main():
    ...
    common = {
        "name": "OAT Weather-Station Listener",
        "version": version,
    ...
    builds = []
    per_board = []
    ENV_FAMILY = {"esp32": "ESP32", "esp32-s3": "ESP32-S3", "esp32-c3": "ESP32-C3", "esp32-c6": "ESP32-C6", "heltec-v2": "ESP32", "heltec-v3": "ESP32-S3", "ibc-v102": "ESP32", "ibc-v102-cc1101": "ESP32"}
    for path in sorted(glob.glob(os.path.join(dest, "firmware-*.bin"))):
        mcu = os.path.basename(path)[len("firmware-"):-len(".bin")]
        fam = ENV_FAMILY.get(mcu) or FAMILY.get(mcu)
        if not fam:
    ...
    # Two boards share each chip family here, so the combined manifest keeps only the
    # devkit builds; the Heltec images are reached by their own per-board buttons.
    _write(os.path.join(dest, "manifest.json"), dict(common, builds=[b for m, b in per_board if not m.startswith(("heltec", "ibc"))]))
