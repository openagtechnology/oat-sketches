...
def after_build(source, target, env):
    ...
    # This sketch has two boards per chip family (a devkit and a Heltec), so images
    # are named by ENV, not by mcu; make_manifest.py maps env -> chipFamily.
    merged = os.path.join(out_dir, "firmware-%s.bin" % env.subst("$PIOENV"))
